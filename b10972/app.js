...
const lastArr = [...beforeArr].sort((a, b) => b - a);

if (beforeArr.every((v, i) => lastArr[i] === v)) {
  //내림차순 정렬한 배열과 요소가 같은 경우 마지막 배열임
  console.log(-1);
} else {
  //뒤에서부터 오름차순이 끝나는 idx === changeAIdx
  let changeAIdx = 0;
  for (let i = n - 1; i >= 0; i--) {
  ...
  //changeAIdx 뒤의 요소들 중 changeAIdx보다는 크면서 뒤의 요소들 중에서 가장 작은수 === changeBVal
  let changeBIdx = 0;
  let changeBVal = Number.MAX_SAFE_INTEGER;
  for (let i = changeAIdx + 1; i < n; i++) {
    if (beforeArr[i] > beforeArr[changeAIdx]) {
      changeBVal = Math.min(changeBVal, beforeArr[i]);
    }
  }
  ...
  //swap후 chaneAIdx 뒤의 요소들을 오름차순 정렬
  let answer = [];
